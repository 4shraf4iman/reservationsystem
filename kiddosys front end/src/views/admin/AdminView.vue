    ...
    return Object.values(registration).some(value => {
      ...
      const formattedChildren = computed(() => {
        return registrations.value.map(registration => {
          const totalChildren = registration.children.length;
          const childrenAges = registration.children.map(child => child.age).join(' y/o, ');
          return `${totalChildren} children: ${childrenAges} y/o`;
        });
      });
